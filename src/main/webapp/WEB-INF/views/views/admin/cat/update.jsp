...
<div class="row">
	<div class="col-md-12 panel-info">
		<div class="content-box-header panel-heading">
			<div class="panel-title ">Cập nhật thông tin danh mục</div>
		</div>
...
<script type="text/javascript">
	...
	var catIdUpdate = url.split('/')[7].split('-')[1];
	...
	async function showCat() {
		...
		if (data.length > 0) {
			...
			for(let item of data){
				// check nếu là cat đang chọn => selected
				if (catParentId == item.catId) {
					selected = 'selected';
				} else {
					selected = '';
				}
				// check nếu là cat đang update => disabled
				if (catIdUpdate == item.catId) {
					disabled = 'disabled';
				} else {
					disabled = '';
				}
				$("#multilevel-cat").append('<option value="'+item.catId+'" '+selected+' '+disabled+'>'+item.catName+'</option>');
		...
		try {
			...
			if (data.length > 0) {
				...
				for(let item of data){
					...
					$("#multilevel-cat").append('<option value="'+item.catId+'" '+selected+' '+disabled+'>'+menu+' '+item.catName+'</option>');
